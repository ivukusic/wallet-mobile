...
import { backgroundColor, border, compose, flexbox, height, layout, space } from 'styled-system';
...
export const Background = styled(LinearGradient).attrs(({ disabled }: { disabled: boolean }) => ({
...
export const Label = styled(CustomText)<CustomTextProps & { customType: string }>`
  font-size: 16px;
...
export const Loading = styled(ActivityIndicator).attrs(({ type }: { type: string }) => ({
  size: 'small',
